...
type (
	productModel interface {
		...
		Query(ctx context.Context, ids string) ([]*Product, error)
	}
...
func (m *defaultProductModel) Query(ctx context.Context, ids string) ([]*Product, error) {
	query := fmt.Sprintf("select %s from %s where `id` in (?) limit 1", productRows, m.table)
	var resp []*Product
